...
Arc2Face takes an ArcFace 512-d vector as input and generates a Stable Diffusion image of a face matching that identity. The conditioning mechanism is subtler than a brief description would suggest, and it is worth getting right because downstream design decisions depend on it. Arc2Face uses the prompt template `"photo of a ⟨id⟩ person"` tokenized into a 5-token sequence `{e₁, e₂, e₃, ŵ, e₅}`. The `⟨id⟩` slot is the single token that carries identity: `ŵ ∈ ℝ^768` is the ArcFace 512-d vector **zero-padded to 768-d** and substituted directly into that token slot. The other four tokens are frozen pseudo-prompt context. The fine-tuning is applied to the CLIP-ViT-L/14 text encoder, which then processes this 5-token sequence through its transformer layers — and this is what the UNet's cross-attention actually conditions on. The paper ablates this design against a 4-layer MLP that learns a projection from ArcFace to CLIP space and finds the fine-tuned-CLIP + direct-substitution approach wins on identity similarity (Fig. 9 of the Arc2Face paper).

Three implications follow from this architecture, none of which are obvious from a summary:

First, there is one 768-d identity slot, not a multi-token identity space. Anyone trying to stack multiple identity channels or factor identity into sub-components has to retrain the pseudo-prompt or add a separate conditioning path via ControlNet or LoRAs — the token bandwidth is effectively one vector.

Second, the text channel is dead after fine-tuning. The paper explicitly states the model "exclusively adheres to ID-embeddings, disregarding its initial language guidance." You cannot stack prompt-based control (CLIP guidance, textual inversion) on top of a vanilla Arc2Face and expect it to work; the CLIP text branch has been repurposed for identity-only conditioning.

Third, the only projection target for downstream consumers is ArcFace 512-d. There is no intermediate "CLIP token space" to project into; the CLIP path is an implementation detail, not an exposed interface. Anyone building on Arc2Face has to project their signal into ArcFace space, zero-padding and substitution happen automatically.

These details matter because they constrain how Arc2Face can be extended. Adding expression control is done via a second cross-attention path (the expression adapter, described below). Adding other axes is done via h-space interventions on the UNet bottleneck, LoRAs trained against Arc2Face, or ControlNets with spatial conditioning — not by adding more prompt tokens.
...
The most concrete published instance of this pattern, as of early 2026, is **Vox2Face** (Di Domenico et al., MDPI *Information* 17(2):200, February 2026). Vox2Face projects a pretrained Audio Spectrogram Transformer's speech embeddings into the ArcFace 512-d hyperspherical identity space via a small MLP mapping network, and drives frozen Arc2Face downstream. The training regime is two-stage: Stage I is supervised alignment of the MLP output to the ArcFace features of the speaker's real face (using a cosine / contrastive loss on the hypersphere), and Stage II is a diffusion self-consistency loss — an SDS-style formulation where the denoising residual acts as implicit critique of whether the predicted identity vector lies on Arc2Face's learned manifold. Only the MLP and a set of LoRA adapters on the Arc2Face UNet receive gradients; the UNet base weights, the VAE, and the ArcFace network itself are frozen. The reported results are modest but real: ArcFace cosine similarity 0.295 → 0.322, R@10 speech-to-face retrieval 29.8% → 32.1%, VGGFace Score 18.82 → 23.21 against an unnamed strong baseline. The paper demonstrates that the "project-arbitrary-embedding-into-ArcFace + frozen Arc2Face" pipeline works end-to-end on a non-trivial input modality.

Vox2Face is also the first published template for the pattern, and its usefulness to a prospective adopter depends on a constraint that must be recognized explicitly: **Vox2Face relies on ground-truth (source, face) pairs.** The Stage I alignment loss is supervised against the speaker's real face, drawn from HQ-VoxCeleb. This is the critical precondition for the recipe to work as-is. For a downstream adopter whose use case has such pairs (speech-identity, video-identity, biometric-identity), Vox2Face transfers directly. For adopters whose use case does not have such pairs — for example, data-visualization applications where the source is a text embedding and the "face" is just a visualization encoding rather than a real biometric — Stage I has to be replaced by a contrastive / distance-preserving objective that does not require ground-truth face targets, typically something like `|d_ArcFace(f(e_A), f(e_B)) − C · d_source(e_A, e_B)| → 0`. Stage II can then be used as-is or dropped entirely if the resulting identity vectors fall within the manifold Arc2Face already handles well. This is the regime where the direct-embedding frontier is still open and essentially unexplored.

A separate architectural sub-question is whether interpolation in ArcFace space is actually smooth enough to support continuous visualization. **Arc2Morph** (Di Domenico et al., arXiv:2602.16569, February 2026) is the closest empirical evidence: it shows that spherical linear interpolation of CLIP-encoder-output token representations produces slightly stronger midpoint morphs than linear interpolation in raw ArcFace space, measured by Morphing Attack Potential on FEI and MONOT benchmarks (gains of ~1-3 percentage points). Arc2Morph does not study smoothness as a metric — there are no LPIPS trajectories, no Lipschitz bounds, no sweep experiments across α ≠ 0.5 — so the "CLIP space is smoother" reading is a hypothesis, not a proven property. For an adopter who needs continuity guarantees, this is a known open empirical question, not a solved one.
...
[13] Di Domenico, N., Franco, A., Ferrara, M., Maltoni, D. "Vox2Face: Speech-Driven Face Generation via Identity-Space Alignment and Diffusion Self-Consistency." MDPI *Information* 17(2):200, February 2026. DOI 10.3390/info17020200. The first published template for projecting a non-face semantic embedding into ArcFace space and driving frozen Arc2Face, using a two-stage regime (cosine alignment + SDS self-consistency).

[14] Di Domenico, N., Franco, A., Ferrara, M., Maltoni, D. "Arc2Morph: Identity-Preserving Facial Morphing with Arc2Face." arXiv:2602.16569, February 2026. The only published ablation comparing ArcFace-space vs CLIP-encoder-output-space interpolation inside the Arc2Face pipeline; demonstrates that slerp in CLIP-output space beats lerp in raw ArcFace space for midpoint morphing on biometric attack metrics, though with no smoothness analysis.

[15] Papantoniou, F.P. et al. Arc2Face paper, Fig. 9 ablation. Direct-substitution + fine-tuned CLIP text encoder wins over a 4-layer MLP baseline on identity similarity, justifying the "zero-padded ArcFace → single token slot" architecture.
