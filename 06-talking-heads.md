...
## The Academic Audio-to-Parameter Lineage

Running in parallel to the pixel-output methods above is an academic tradition of audio-driven *parameter-output* methods — networks that consume speech and emit either FLAME parameters, custom mesh vertex offsets, or ARKit-compatible blendshape coefficients, rather than RGB frames. These models target an entirely different consumer: a downstream rig that wants animation parameters to apply to its own renderer, not a finished video. They are the academic counterparts of NVIDIA Audio2Face, and they matter for any pipeline that already owns a Live2D, MetaHuman, FLAME-3DMM, or 3DGS renderer and needs only the audio-to-motion bridge.

**FaceFormer** (Fan et al., CVPR 2022) [21] was the first widely-cited transformer-based audio-to-3D-face system. It is autoregressive over mesh frames and operates on the VOCASET and BIWI datasets at FLAME-registered topology; the released pretrained models emit a 15069-dimensional per-frame vertex vector. There is no explicit blendshape output — downstream consumers must fit ARKit-52 or FLAME-expression coefficients onto the predicted meshes if they want parametric output.

**CodeTalker** (Xing et al., CVPR 2023) [22] replaced the continuous regression target with a discrete motion-codebook prior trained via VQ-VAE, then conditioned audio-to-codebook generation on the speech features. The motion-codebook trick mitigates the over-smoothing that plagued earlier regression models — the discrete prior forces the network to commit to a recognizable motion rather than interpolate between training samples. Outputs are FLAME-registered meshes (VOCASET) or BIWI meshes; no canonical ARKit export.

**MeshTalk** (Richard et al., ICCV 2021) [23] from Facebook Reality Labs is the lineage ancestor — categorical latent expression code, mesh-vertex output (custom topology, not FLAME), OBJ-format runtime. The code is published under CC-NC 4.0; the categorical-latent design predates and motivated CodeTalker's codebook approach.

**EmoTalk** (Peng et al., ICCV 2023) [24] is the academic model worth flagging specifically: it explicitly outputs 52 ARKit-compatible blendshape coefficients and includes an emotion-disentangling encoder that allows the emotional valence of the output to be controlled separately from the audio input. It is the only widely-cited academic audio-to-parameter model that emits the production-standard ARKit-52 format directly. The released code and weights are under CC BY-NC 4.0 — non-commercial — which is a real constraint for product deployment but makes EmoTalk the cleanest research starting point when ARKit-52 output is needed without standing up the NVIDIA NIM stack.

**DiffPoseTalk** (Sun et al., SIGGRAPH 2024) [25] applied diffusion to the audio-to-parameter task and added explicit head-pose prediction alongside expression — earlier methods left global rotation as someone else's problem. DiffPoseTalk predicts FLAME expression coefficients (ψ), jaw, and global rotation; the released model runs at approximately 30 FPS on an RTX 3090. The live-streaming pipeline incurs a multi-second window-fill delay (the paper reports 7.33 s total for a 4 s context window plus 3.33 s processing), so DiffPoseTalk is offline-grade for low-latency interactive use but is the cleanest FLAME-parameter publication with both expression and pose in a single model.

**UniTalker** (Fan et al., 2024) [26] is a multi-dataset unified audio-to-FLAME model. The published checkpoints depend on FLAME 2020 (`generic_model.pkl`) and target FLAME expression and jaw parameters. UniTalker has no native ARKit-52 export and shares the same FLAME-to-ARKit retargeting requirement as FaceFormer / CodeTalker / DiffPoseTalk before it can drive a Live2D or MetaHuman rig.

The pattern across this lineage is that output format is the load-bearing design decision, not the architecture. FaceFormer / CodeTalker / MeshTalk / DiffPoseTalk / UniTalker all emit mesh vertices or FLAME parameters, which means consuming them from a Cubism or ARKit-targeted pipeline requires an additional FLAME-to-ARKit-52 fit (Chapter 09). EmoTalk emits ARKit-52 directly, which is why it remains the academic reference for audio-to-blendshape pipelines despite being two years older than DiffPoseTalk. NVIDIA Audio2Face-3D (next section) is the production-grade extension of the EmoTalk side of this branch, with the same parameter-output commitment but a microservice deployment story attached.

## NVIDIA Audio2Face and the Proprietary Ecosystem

**NVIDIA Audio2Face** (originally part of NVIDIA Omniverse, open-sourced on September 24, 2025) [14] is a production-grade audio-to-blendshape pipeline that has been deployed commercially for several years inside Omniverse. It takes audio input and produces a stream of ARKit-compatible blendshape values that can drive a MetaHuman, a Unity character, or any rig that accepts ARKit. NVIDIA's September 2025 open-source release included the SDK, Maya and Unreal Engine 5 plugins, the training framework (v1.0), a regression model (v2.2) and a diffusion model (v3.0), and the companion Audio2Emotion model. NVIDIA's NIM service wraps Audio2Face to output ARKit blendshapes directly. As of the v3.0 Hugging Face model card (`nvidia/Audio2Face-3D-v3.0`, dated 2025-09-24), the published model is governed by the NVIDIA Open Model License, which permits commercial use under the license terms — distinguishing Audio2Face-3D from the academic ARKit-52 model EmoTalk, whose CC BY-NC 4.0 license blocks commercial deployment. Two operational caveats are worth flagging: the v3.0 microservice exposes ARKit-named blendshape channels, but several head-rotation, eye, and tongue channels are documented as always-zero (the microservice expects downstream consumers to drive head and eye motion separately), and the historical pre-v3 model used a 46-blendshape template that did not align one-to-one with ARKit-52. New integrations should target v3.0 explicitly.
...
| NVIDIA Audio2Face | 2019+, OSS Sept 2025 | audio | ARKit blendshape output only | real-time | production-grade parameters | open (NVIDIA Open Model License) | rig integration |
| FaceFormer | CVPR 2022 | audio | FLAME-mesh vertex output | offline-grade | smooth but mean-tendency | public | academic baseline |
| MeshTalk | ICCV 2021 | audio | custom-mesh vertex output | offline-grade | categorical-latent expression | public (CC BY-NC) | academic baseline |
| CodeTalker | CVPR 2023 | audio | FLAME-mesh vertex output | offline-grade | discrete motion prior fixes over-smoothing | public | research |
| EmoTalk | ICCV 2023 | audio | ARKit-52 blendshape output | offline-grade | emotion-disentangled | public (CC BY-NC) | research, non-commercial only |
| DiffPoseTalk | SIGGRAPH 2024 | audio | FLAME params + head pose | 30 FPS, ~7 s live latency | head pose included | public | research with head pose |
...
[16] Wang, M., Wang, Q., Jiang, F., Xu, M. "FantasyTalking2: Timestep-Layer Adaptive Preference Optimization for Audio-Driven Portrait Animation." arXiv:2508.11255, August 2025. Code: `github.com/Fantasy-AMAP/fantasy-talking2`. First RLHF pipeline for talking heads: Talking-Critic reward model, Talking-NSQ 410K preference dataset, TLPO preference-optimization framework. AAAI 2026 acceptance claim in secondary sources was not corroborated against the official AAAI 2026 proceedings page on 2026-04-14; treat venue as arXiv-only.
...
[21] Fan, Y., Lin, Z., Saito, J., Wang, W., Komura, T. "FaceFormer: Speech-Driven 3D Facial Animation with Transformers." CVPR 2022. `github.com/EvelynFan/FaceFormer`. Autoregressive transformer over FLAME-registered (VOCASET) or BIWI mesh vertex frames. Pretrained `biwi.pth` and `vocaset.pth` available from the repo; no ARKit-52 export.

[22] Xing, J., Xia, M., Zhang, Y., Cun, X., Wang, J., Wong, T.-T. "CodeTalker: Speech-Driven 3D Facial Animation with Discrete Motion Prior." CVPR 2023. `github.com/Doubiiu/CodeTalker`. VQ-VAE discrete motion codebook addressing over-smoothing; mesh outputs at FLAME-registered topology.

[23] Richard, A., Zollhöfer, M., Wen, Y., de la Torre, F., Sheikh, Y. "MeshTalk: 3D Face Animation from Speech using Cross-Modality Disentanglement." ICCV 2021. `github.com/facebookresearch/meshtalk`. Facebook Reality Labs; categorical latent expression code, OBJ mesh output, CC BY-NC 4.0.

[24] Peng, Z., Wu, H., Song, Z., Xu, H., Zhu, X., He, J., Liu, H., Fan, Z. "EmoTalk: Speech-Driven Emotional Disentanglement for 3D Face Animation." ICCV 2023. `github.com/psyai-net/EmoTalk_release`, paper `openaccess.thecvf.com/content/ICCV2023/papers/Peng_EmoTalk_Speech-Driven_Emotional_Disentanglement_for_3D_Face_Animation_ICCV_2023_paper.pdf`. **Explicit 52 ARKit-compatible blendshape coefficient output** with disentangled emotion conditioning. Release under CC BY-NC 4.0; non-commercial-only.

[25] Sun, Z., Lv, T., Ye, S., Lin, M. G., Sheng, J., Wen, Y.-H., Yu, M., Liu, Y.-J. "DiffPoseTalk: Speech-Driven Stylistic 3D Facial Animation and Head Pose Generation via Diffusion Models." SIGGRAPH 2024. `github.com/DiffPoseTalk/DiffPoseTalk`, project page `diffposetalk.github.io`. FLAME expression (ψ), jaw, and global rotation prediction; ~30 FPS generation on RTX 3090; live-streaming pipeline incurs ~7.33 s total latency from window-fill plus processing.

[26] "UniTalker: Scaling up Audio-Driven 3D Facial Animation through A Unified Model." `github.com/X-niper/UniTalker`, project page `x-niper.github.io/projects/UniTalker/`. Multi-dataset unified model targeting FLAME 2020 expression and jaw parameters; no native ARKit-52 export.

See also `vamp-interface/docs/research/2026-04-12-3dmm-flame-diffusion-vtuber-realtime.md` for the underlying research that this chapter draws on for the talking-head section, especially the comparison tables for fastest production-grade methods, and `vamp-interface/docs/research/2026-05-11-speech-to-blendshape-models.md` for the per-model audit (output formats, licenses, runtime requirements) of the academic audio-to-parameter lineage.
