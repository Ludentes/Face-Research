...
## Use Case 10: Stylized 2D / Mascot / Non-Human Talking Head

**Context.** A product needs a stylized 2D character — a brand mascot, an indie game NPC, an AI VTuber, an accessibility avatar, a talking bear — that speaks from audio (TTS, microphone, or pre-recorded narration) without a face camera behind it. The character has its own art style (cartoon, anime, painted, snout-bearing animal) that the realistic-face pixel-output methods cannot match. The renderer is fixed to a 2D rigged stack — Live2D Cubism in the dominant case, occasionally Inochi2D or a custom rig — and the only question is how to drive it from audio.

The defining constraint is that the character is *not photorealistic and not human-topology*. This rules out the talking-head pixel methods of Chapter 06 (EMO, Hallo, FLOAT, MuseTalk, SadTalker, Audio2Photoreal, VASA-1) — they synthesize photoreal frames against a reference image and have no path into a 2D rigged renderer. The remaining options are parameter-output: drive the Cubism parameters directly.

**Recommendation (most boring, ships today, no ML stack):** Cubism Motion-Sync for the mouth (vowel viseme bake or live), plus procedural overlays for everything else — an auto-blink loop on `ParamEyeLOpen` / `ParamEyeROpen`, idle perlin noise on `ParamAngleX` / `ParamAngleY` / `ParamAngleZ`, and scripted expression triggers (`.exp3.json` fades) keyed off emotion tags emitted by the TTS or LLM pipeline. For Cubism 5.0 rigs use the bundled CRI LipSync for higher-fidelity mouth. Rhubarb Lip Sync is the alternative when an offline pre-baked viseme track on disk is preferred over a runtime audio analyzer (indie game dialog systems with fixed scripts). This stack has zero inference cost, runs anywhere the Cubism runtime runs, and is the path most production VTuber tools default to when face tracking is unavailable.

**Recommendation (rich, full-face animation, commercial-OK):** NVIDIA Audio2Face-3D (v3.0 microservice or Hugging Face checkpoint `nvidia/Audio2Face-3D-v3.0`, NVIDIA Open Model License) emitting ARKit-52 blendshape streams, fed into VTube Studio over its iPhone-tracking input port using a LiveLinkFace UDP emulator (the same protocol an iPhone Live Link Face app would speak), with the per-rig ARKit-to-Cubism-parameter mapping authored once inside VTube Studio's UI. This gives mouth, eyes, brows, jaw, and emotion from audio alone. Cost: the Audio2Face-3D microservice requires Ubuntu 22.04 + CUDA 12.1 + Docker + NVIDIA Container Toolkit + an RTX-class GPU.

**Recommendation (research demo, non-commercial):** EmoTalk (ICCV 2023, `github.com/psyai-net/EmoTalk_release`, CC BY-NC 4.0) emitting 52 ARKit-compatible blendshape coefficients with explicit emotion control, bridged into VTube Studio via the same LiveLinkFace path. EmoTalk is the cleanest academic model for this use case because it is the only widely-cited audio-to-parameter model that emits ARKit-52 directly. The CC BY-NC 4.0 license blocks commercial deployment.

**Why not something else?**
- *Pixel-output talking heads (EMO, Hallo, FLOAT, MuseTalk, SadTalker, Audio2Photoreal, VASA-1, AniTalker, GeneFace)* — wrong abstraction. They synthesize realistic video frames against a reference image, not animation parameters; there is no path into a 2D rigged renderer that preserves the rig's art style.
- *FaceFormer / CodeTalker / MeshTalk / DiffPoseTalk / UniTalker* — these are mesh-vertex or FLAME-parameter outputs, not ARKit-52, so consuming them from a Cubism rig requires an additional FLAME-to-ARKit-52 fit (Chapter 09) and a per-rig mapping. Almost always the wrong starting point versus EmoTalk or Audio2Face-3D, which already speak ARKit-52.
- *LivePortrait or PersonaLive* — these are *visual* drivers (video-to-portrait warp); they consume a driving face video, not audio. Wiring audio in requires a separate audio-to-driver-face stage (an inverse rendering problem) and degrades to a multi-stage chain with quality loss. Wrong tool for this use case unless the audio path is unrelated and a separate driving video is already in hand.
- *3DGS avatars* — wrong art style. 3DGS targets photorealistic-or-near-photorealistic identities and requires multi-view capture of a real person or a 3D-modeled character; it does not produce stylized 2D output.

**Pitfalls.** The ARKit-52 schema assumes human lip topology. A snout, beak, or non-mammalian mouth does not have meaningful semantics for `mouthDimple`, `mouthRollUpper`, `mouthRollLower`, `cheekPuff`, `cheekSquint`, or `noseSneer`; bake those channels to zero in the per-rig mapping or remap them broadly into "snarl" or "smile" macros. The load-bearing channels for a non-human character are `jawOpen`, `mouthFunnel`/`mouthPucker`, `mouthSmile_L`/`mouthSmile_R`, `mouthFrown_L`/`mouthFrown_R`, `eyeBlink_L`/`eyeBlink_R`, `eyeLookUp` / `Down` / `In` / `Out`, `browInnerUp`, `browDown_L`/`browDown_R`, plus head pose; everything else is dead weight or should be remapped to art-specific macros. The NVIDIA Audio2Face-3D v3.0 microservice documents that several head-rotation, eye, and tongue channels are always-zero — drive head pose from a separate idle-motion module or from the LLM/TTS layer, not from the Audio2Face stream. Test cross-language: most published models are English-trained and produce wrong vowel shapes for tonal or non-Latin languages.

## Use Case 11: Research on Face Animation Itself
