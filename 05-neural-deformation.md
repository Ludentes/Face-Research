...
**The photoreal-skin bias has an architectural locus.** For anyone fine-tuning a LivePortrait-class generator toward stylised output: the bias is concentrated in the decoder G's SPADE γ/β projection layers. The next subsection treats this in depth and lays out the FFHQ-StyleGAN2 transfer-learning ecosystem that makes a surgical fine-tune practical without retraining the full LP corpus.

## Stylized anchors, SPADE locus, and the FFHQ-StyleGAN2 layer-swap pool

LivePortrait's training corpus is photoreal-skewed by roughly three orders of magnitude — approximately 69 million photoreal video frames against ~60K AAHQ stylised stills [2] — and the resulting generator carries a strong photoreal-skin prior. Heavily stylised anchors (paintings, hand-drawn illustrations, anime portraits) fail in two architecturally distinct modes: (a) **collapse**, where the appearance encoder F and warp module W produce geometric tearing on out-of-distribution input, and (b) **photoreal-leak**, where geometry survives but the decoder G's photoreal-skin prior dominates and the anchor's brushwork, hatching, or flat-tone character is lost. Mode (a) is W's failure; no amount of decoder retraining fixes it (see the compositional-split pattern in the operational notes above). Mode (b) is G's failure, and there is a productive line of attack that does not require a full LP retrain.

### Where stylization lives, mechanically

Three converging arguments locate the photoreal-leak failure mode in G's SPADE [34] γ/β projection layers specifically. First, Gatys et al. [35] established that perceptual style is a *channel-statistics* property — Gram matrices over feature-map channel co-occurrence at early-to-mid network depth — not a geometric one. Style transfer modifies the per-channel mean and variance of activations; content survives at the level of which channels fire where, but the *magnitude* of channel firings is what carries brushwork, palette, and surface character. Second, the StyleGAN coarse-to-fine resolution-to-attribute map [1, Ch08] places identity and pose at low resolution and texture/skin/colour at high resolution. Pinkney's Toonify layer-swap [36] is the cleanest empirical confirmation: swapping the high-resolution layers between an FFHQ-StyleGAN and a toonified-StyleGAN produces a toon-textured photoreal-structured face, while the inverse swap produces an output that is *"deeply weird"* (Pinkney's phrasing) — the two failure modes are not symmetric, and texture lives in the high-resolution layers. SPADE in LP's G is precisely the mechanism that injects channel-affine modulation into the decoder's high-resolution activations: spatially-adaptive AdaIN [37], where the γ/β are predicted from the warped feature map rather than from a class label. Third, CustomDiffusion [38] established that fine-tuning only the cross-attention K/V projections (≈3% of total parameters) is sufficient for full DreamBooth-style personalization in a diffusion model — a strong precedent for surgical small-parameter fine-tuning at the point where conditioning enters the network. SPADE's γ/β projections are the architectural analog of cross-attention K/V in encoder-decoder GAN-style generators: they are where external conditioning becomes channel-wise modulation. Targeting them with a LoRA is the natural translation of the CustomDiffusion recipe to LP-class generators.

The practical conclusion is that a stylised LivePortrait variant probably does not need a full retrain. A LoRA on G's SPADE γ/β projections, trained on identity-preserved photo↔styled pairs, should be sufficient. The bottleneck shifts from "do we have ~69M stylised video frames" (we do not) to "can we generate identity-preserved photo↔styled pairs at scale" (we can).

### The paired-synthetic corpus pattern

The key resource is the FFHQ-StyleGAN2 transfer-learning ecosystem. Karras et al.'s StyleGAN2-ADA [39] established that a StyleGAN2 head trained from scratch on a small specialised dataset (≥1K images) by transfer-learning from FFHQ retains a meaningful semantic correspondence with the base FFHQ model in W-space. The same latent code `W` rendered through the FFHQ base and through a transfer-trained style head produces two images that are recognisably the same person in two different visual idioms. Several such style heads exist publicly:

- **MetFaces** [39]: 1,336 Met Museum portrait paintings, 1024², official NVIDIA, license-clean for non-commercial research.
- **Ukiyoe Faces** [36, model card]: 5,000 Japanese woodblock-print faces at 256², trained by Pinkney, license-clean (out of copyright).
- **DualStyleGAN** [40]: Yang et al. CVPR 2022, with ten public style heads — Cartoon, Caricature, Anime, Arcane, Comic, Pixar, Slamdunk, Fantasy, Illustration, Impasto — plus an 18-element per-resolution-layer style-blending weight knob.
- **More Abstract Art** [Saraev model card]: 14K abstract paintings at 512², community-released.
- **NeuralKuvshinov v2 / v1.27** [41]: Ilya Kuvshinov–style anime illustration heads (`github.com/dobrosketchkun/NeuralKuvshinov_v2`); useful as a research probe, license-fraught for shipping.

All of these are FFHQ-StyleGAN2-transfer-trained and Pinkney-layer-swap-compatible *within* the family. They are *not* layer-swap-compatible with StyleGAN3, with SDXL, or with LP itself — Pinkney layer-swap is intra-architecture-family only [36]. The mistake to avoid is expecting that a stylised StyleGAN3 head can be combined with FFHQ-StyleGAN2 weights, or that any StyleGAN swap maps directly onto LP's decoder. The transfer happens through *generated images*, not through weights.

The corpus pipeline that follows is straightforward:

```
For each latent code W ∈ ℝ^512 (sample 10⁵–10⁶):
    photo  = FFHQ_StyleGAN2(W)         # photoreal identity at W
    styled = StyleHead_StyleGAN2(W)    # same identity, target style
    pair   = (photo, styled)            # identity preserved by W-anchor
→ Train a LoRA on LP-G's SPADE γ/β projections under a paired-image objective
  (perceptual loss + adversarial loss on the styled side, with G's warping
  path frozen and driver = same).
```

Each style head produces an arbitrarily large synthetic corpus in its own style with identity coherence guaranteed by the shared W. MetFaces and Ukiyoe give license-clean shippable style packs; NeuralKuvshinov gives a higher-quality research probe to validate the pipeline before committing to packs.

### Comparison to other transfer pathways

The paired-synthetic-corpus pattern sits in the middle of a broader data-curve for style-transfer training:

- **StyleGAN-NADA** [42]: zero source images required; uses CLIP text supervision to walk a StyleGAN's weights toward a text-described style. Cheap, but constrained to what CLIP can describe.
- **JoJoGAN** [43]: one source image; fits a per-anchor stylization network in ~30 minutes via a learned-direction-from-W-inversion mechanism. The natural reference point for "per-anchor fit at upload" product mechanics on top of LP.
- **Mind the Gap** [Ojha et al. CVPR 2021]: 1–10 source images.
- **AgileGAN / DualStyleGAN / Toonify**: 100–300 source images, full transfer-trained head.
- **LP-animals** [2]: ~230K source frames, base modules fine-tuned, stitch/retarget skipped.

The FFHQ-StyleGAN2 layer-swap pool sits in the 100–300-image-per-head tier (where the heads were trained) and provides the synthetic paired data for the LP-side fine-tune at effectively unbounded scale. The combination is what makes the LP-G SPADE LoRA path practical without collecting a stylised video corpus.

### Open status as of mid-2026

No public stylised LivePortrait variant exists. The closest published precedent is LP-animals — a base-module fine-tune of LP on ~230K cat/dog video frames — which serves as existence proof that LP's base modules accept domain-transfer fine-tunes, but stops short of demonstrating the surgical SPADE-γ/β-only LoRA recipe on a paired-synthetic corpus. The research direction is open. The compositional-split pattern (operational notes above) is the production workaround that ships today; the SPADE-LoRA path is the quality lift that becomes possible when the keystone paired-synthetic corpus pipeline closes.
...
[34] Park, T., Liu, M.-Y., Wang, T.-C., Zhu, J.-Y. "Semantic Image Synthesis with Spatially-Adaptive Normalization" (SPADE / GauGAN). CVPR 2019, arXiv:1903.07291. The normalization scheme used in LivePortrait's decoder G: γ and β are predicted per-spatial-location from a conditioning input (semantic mask in SPADE's original use, warped feature map in LP's use), then applied as a channel-wise affine modulation. The architectural injection point for the stylization-locus argument.

[35] Gatys, L. A., Ecker, A. S., Bethge, M. "Image Style Transfer Using Convolutional Neural Networks." CVPR 2016. The foundational result that style is captured by Gram matrices over feature-map channel co-occurrence statistics at early-to-mid CNN depth, while content survives at deep semantic activation. The mechanistic basis for treating photoreal-skin bias as a channel-statistics phenomenon rather than a geometric one.

[36] Pinkney, J., Adler, D. "Resolution Dependent GAN Interpolation for Controllable Image Synthesis Between Domains" (Toonify). NeurIPS 2020 Workshop on Machine Learning for Creativity and Design. The empirical confirmation that StyleGAN's resolution layers carry attributes hierarchically — coarse layers carry identity/pose, fine layers carry texture/colour — and that intra-architecture-family layer-swap between FFHQ and a transfer-trained style head is the cheapest way to combine the two. Companion writeup: `www.justinpinkney.com/blog/toonify`. The Ukiyoe Faces and other model cards live at `github.com/justinpinkney/awesome-pretrained-stylegan2`.

[37] Huang, X., Belongie, S. "Arbitrary Style Transfer in Real-time with Adaptive Instance Normalization" (AdaIN). ICCV 2017, arXiv:1703.06868. The ancestor of SPADE: channel-wise mean/variance alignment as the operative style-transfer mechanism. SPADE generalizes AdaIN to spatially-adaptive γ/β.

[38] Kumari, N., Zhang, B., Zhang, R., Shechtman, E., Zhu, J.-Y. "Multi-Concept Customization of Text-to-Image Diffusion" (CustomDiffusion). CVPR 2023. Established that fine-tuning only the cross-attention K/V projection layers (≈3% of total parameters) is sufficient for full DreamBooth-style personalization. The strong precedent for surgical small-parameter fine-tuning at the conditioning-injection point of a generative network. SPADE γ/β are the GAN-encoder-decoder analog of the K/V projections.

[39] Karras, T., Aittala, M., Hellsten, J., Laine, S., Lehtinen, J., Aila, T. "Training Generative Adversarial Networks with Limited Data" (StyleGAN2-ADA). NeurIPS 2020. The transfer-learning recipe that produces FFHQ-compatible style heads from ≥1K images. The accompanying MetFaces dataset (1,336 Met Museum portrait paintings) became the canonical license-clean painterly style head and the first entry in the FFHQ-StyleGAN2 layer-swap pool.

[40] Yang, S., Jiang, L., Liu, Z., Loy, C. C. "Pastiche Master: Exemplar-Based High-Resolution Portrait Style Transfer" (DualStyleGAN). CVPR 2022. Ships ten public style heads (Cartoon, Caricature, Anime, Arcane, Comic, Pixar, Slamdunk, Fantasy, Illustration, Impasto) plus an 18-element per-resolution-layer style-blending knob. Code: `github.com/williamyang1991/DualStyleGAN`.

[41] dobrosketchkun. "NeuralKuvshinov v2." Public GitHub repository at `github.com/dobrosketchkun/NeuralKuvshinov_v2`. Ilya Kuvshinov-style StyleGAN2-ADA-PyTorch head, FFHQ-transfer-trained, multiple checkpoint variants (v1.27, v2.1, v2.2, v2.3) plus male/megane/sketch specialty heads. License-fraught — Kuvshinov did not consent to having his portraiture style cloned — so usable as a research probe to validate the paired-synthetic-corpus pipeline but not as a shippable style pack.

[42] Gal, R., Patashnik, O., Maron, H., Bermano, A., Chechik, G., Cohen-Or, D. "StyleGAN-NADA: CLIP-Guided Domain Adaptation of Image Generators." ACM TOG 2022, arXiv:2108.00946. Zero source images: walks a StyleGAN's weights toward a CLIP-text-described style. The cheapest-data end of the style-transfer training curve; constrained to what CLIP can describe but useful as a baseline reference for "style transfer with no target images."

[43] Chong, M. J., Forsyth, D. "JoJoGAN: One Shot Face Stylization." ECCV 2022, arXiv:2112.11641. Per-anchor stylization fit in ~30 minutes from a single source image via W-inversion plus a learned style direction. The reference point for "per-anchor fit at upload" product mechanics on top of any LP-class generator; sits between StyleGAN-NADA (0 images) and DualStyleGAN (100–300 images) on the data-curve.
