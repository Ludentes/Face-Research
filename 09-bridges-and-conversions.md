...
## From Audio (Speech Waveform)

Speech audio is the entry point for the talking-head pipeline (Chapter 06) and is increasingly a primary source representation in production VTubing and AI-character systems. The audio-to-motion bridges form a complete sub-graph that is worth enumerating here because the chapter on talking heads emphasizes the rendering side, not the bridge structure.

**Audio → ARKit blendshapes.** Run NVIDIA Audio2Face-3D (microservice or v3.0 Hugging Face checkpoint) or the academic EmoTalk model. Cost: ~33 ms per frame at the documented 30 fps target on RTX-class hardware; the NVIDIA microservice requires Ubuntu 22.04 + CUDA 12.1 + Docker + NVIDIA Container Toolkit. Quality: production-grade for Audio2Face-3D; competitive for EmoTalk with explicit emotion control. License: NVIDIA Open Model License for Audio2Face-3D (commercial OK under terms); CC BY-NC 4.0 for EmoTalk (non-commercial only). The microservice and v3.0 checkpoint output ARKit-named channels; a subset of head, eye, and tongue channels in the v3.0 microservice are documented as always-zero and require separate driving.

**Audio → Live2D parameters (mouth-only loudness).** Cubism SDK's built-in `CubismAudioMouthInput` reads audio RMS and writes `ParamMouthOpenY` per frame. Cost: microseconds. Quality: mouth-open amplitude only — no vowel shape. Availability: built into the Cubism SDK in all supported runtimes.

**Audio → Live2D parameters (vowel viseme bake).** Cubism Editor's Motion-Sync analyzes a `.wav` file and produces a `.motionsync3.json` track containing A/I/U/E/O vowel coefficients plus a generic mouth-deformation channel; the `CubismMotionSyncController` plays the baked track. Cost: offline analysis at authoring time, microseconds per frame at runtime. Quality: vowel-level mouth shape; no eyes, brows, or head. Availability: Cubism Editor + Unity MotionSync plugin (`github.com/Live2D/CubismUnityMotionSyncComponents`). CRI LipSync, bundled in Cubism 5.0, is a higher-quality real-time variant of the same path.

**Audio → mouth shape labels (offline CLI).** Rhubarb Lip Sync (`github.com/DanielSWolf/rhubarb-lip-sync`, Boost 1.0) emits a discrete sequence of 6 mouth-shape labels (plus three optional extended) given an audio file. Cost: offline analysis at authoring time. Quality: phoneme-aligned mouth labels; no continuous coefficients. Availability: command-line tool, CPU-only, no GPU. The standard lightweight option for indie game dialog systems and pre-baked Live2D tracks.

**Audio → FLAME parameters.** Run DiffPoseTalk, FaceFormer (with downstream FLAME-fit), CodeTalker, or UniTalker. Cost: 30 FPS on RTX 3090 for DiffPoseTalk's generation step, but the live-streaming pipeline incurs a ~7 s window-fill plus processing delay. Quality: research-grade; FLAME expression coefficients plus, in DiffPoseTalk's case, head pose. Availability: academic open code, mostly without explicit licenses. FLAME outputs need an additional FLAME → ARKit-52 fit (above) before they can drive a Live2D or MetaHuman rig.

**Audio → custom-mesh vertex offsets.** Run FaceFormer or MeshTalk. Cost: offline-grade. Quality: smooth but mean-tendency for FaceFormer, categorical-latent for MeshTalk. Availability: research code with pretrained checkpoints; mesh topology is custom (MeshTalk) or FLAME-registered (FaceFormer/CodeTalker) — there is no ARKit export and downstream consumers must fit blendshape coefficients to the predicted meshes if parametric output is required. Almost always the wrong choice for a Live2D or MetaHuman target pipeline; included here to flag the "famous paper, wrong abstraction" trap.

**Audio → pixels (talking-head video).** Run EMO, Hallo, FLOAT, MuseTalk, SadTalker, or one of the other Chapter 06 pixel-output methods. Cost: seconds per second of video offline (EMO, Hallo), ~30 FPS real-time for MuseTalk (mouth region only) and SadTalker. Quality: state of the art for the pixel-output formulation. Availability: open code for the open-source members of the list. Architecturally distinct from the bridges above — pixel-output methods do not produce a parameter stream that can drive an external rig and so cannot be plugged into a Live2D, MetaHuman, or VRM pipeline.

The audio-out-to-Live2D sub-graph is the bridge structure relevant to AI VTubers, mascot characters, and stylized 2D talking-head deployments. The two ends of the cost-quality spectrum are Cubism Motion-Sync (zero ML inference, mouth only, ships in the SDK) and NVIDIA Audio2Face-3D fed through a LiveLinkFace UDP emulator into VTube Studio's iPhone-tracking input port (full ARKit-52, emotional, requires Docker stack). There is no canonical mapping table between ARKit-52 and Live2D parameters published anywhere; the production reality is that VTube Studio's per-rig mapping UI is where the bridge is authored, once per Live2D model, by the rigger.
