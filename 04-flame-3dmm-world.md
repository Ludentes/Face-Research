...
**RigFace** (arXiv:2502.02465, February 2025, latest revision November 2025) fully fine-tunes SD 1.5 with DECA-derived 3D renders, expression coefficients, and masks as conditioning. Identity is preserved via a full-UNet Identity Encoder rather than a small adapter, and the full backbone is trained rather than just a plugin module. The result is high-fidelity editing with clean disentanglement of expression, pose, and lighting. **Availability is weights-only**: pretrained weights are on HuggingFace under `mengtingwei/rigface` (Apache-2.0), but no inference code or training code has been released, and the arXiv paper itself advertises no repository — see Chapter 08 for details.
